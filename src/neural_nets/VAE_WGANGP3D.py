...
"""
Implementation of the 3D VAE/WGAN-GP architecture.
Very similar to the WGAN-GP the only change is that in train step the latent manifold from pretrained VAE
is given as an input to the generator.
"""

def make_VAE_WGANGP3D_critic(input_shape: int=128):
    """
    VAE/WGAN-GP critic function with shapes appropriate for the skull implants problem.
...
def make_VAE_WGANGP3D_generator(latent_dim: int=200, hidden_dim: int=512, output_shape: int=128):
    """
    VAE/WGAN-GP generator function with shapes appropriate for the skull implants problem.
...
# Like a reparametrization trick
def get_latent_vector(Z_mean, Z_var, batch_size):    
...
class VAE_WGANGP3D_model(models.Model):
    def __init__(self, generator, critic, critic_extra_steps, lambda_gp, Z_mean_path, Z_var_path):
        super(VAE_WGANGP3D_model, self).__init__()
        self.generator = generator
        self.critic = critic
        self.critic_extra_steps = critic_extra_steps
        self.lambda_gp = lambda_gp
        self.Z_mean = pickle.load(open(Z_mean_path, "rb"))
        self.Z_var = pickle.load(open(Z_var_path, "rb"))
    
    def compile(self, generator_optimizer, critic_optimizer, generator_loss_fn, critic_loss_fn):
        super(VAE_WGANGP3D_model, self).compile()
    ...
    def train_step(self, real_data):
        ...
        for _ in range(self.critic_extra_steps):
            vae_latent = get_latent_vector(self.Z_mean, self.Z_var, batch_size)
            with tf.GradientTape() as tape:
                fake_data = self.generator(vae_latent)
        ...
        # Train the generator
        vae_latent_data = get_latent_vector(self.Z_mean, self.Z_var, batch_size)
        with tf.GradientTape() as tape:
            predictions = self.critic(self.generator(vae_latent_data))
...
class VAE_WGANGP3D_monitor(keras.callbacks.Callback):
    def __init__(self, save_path, Z_mean_path, Z_var_path):
        self.save_path = save_path
        self.Z_mean = pickle.load(open(Z_mean_path, "rb"))
        self.Z_var = pickle.load(open(Z_var_path, "rb"))

    def on_epoch_end(self, epoch, logs=None):
        vae_latent = get_latent_vector(self.Z_mean, self.Z_var, 1)
        generated_skull = self.model.generator(vae_latent)
